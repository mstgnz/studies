...
jQuery(document).ready(function() {
    ...
    if (!$.cookie('start_counts')) {
        box.keypress(countCharsAndWordsCounts).blur(countCharsAndWordsCounts).focus(countCharsAndWordsCounts).change(countCharsAndWordsCounts).keyup(countCharsAndWordsCounts).keydown(countCharsAndWordsCounts).load(countCharsAndWordsCounts);
        box.focus();
    } else {
        box.keypress(countEditedCharsAndWordsCounts).blur(countEditedCharsAndWordsCounts).focus(countEditedCharsAndWordsCounts).change(countEditedCharsAndWordsCounts).keyup(countEditedCharsAndWordsCounts).keydown(countEditedCharsAndWordsCounts).load(countEditedCharsAndWordsCounts);
        box.focus();
    }
    ...
    function countCharsAndWordsCounts() {
        if (flag) {
            ...
            text = box.val();
            $.cookie('text_for_edit_counter', text, {
                expires: 365
            });
            ...
            $.cookie('start_counts', JSON.stringify(cookieObject), {
                expires: 365
            });
        }
    }
    ...
    function countEditedCharsAndWordsCounts() {
        ...
        if (counts.chars != undefined && counts.words != null) {
            ...
            $('.characters').text(0);
            $('.words').text(0);
            if (cookieObject.start_character_count == 0) $('.edited_characters').text('+0');
            else $('.edited_characters').text(0 - cookieObject.start_character_count);
            if (cookieObject.start_word_count == 0) $('.edited_words').text('+0');
            else $('.edited_words').text(0 - cookieObject.start_word_count);
        }
    }
