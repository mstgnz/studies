...
class Curl {
	...
	function __construct($apikey, $url, $cache=false){
		...
		curl_setopt($this->ch, CURLOPT_TIMEOUT_MS, 24000);
	...
	public function get($action, $cache=false){
		...
		if(!$result || $cache){
			...
			$this->cacheList($this->cacheKey($action));
	...
	private function setCache($cache){
		if($cache){
			if(!empty($_GET['key']) || !empty($_GET['refresh'])){
				$cache = false;
			}
		}
		return $cache;
	...
	private function cacheList($key){
	...
	// user ip adresini al
	private function getIpAddressPoint(){ // noktalı
